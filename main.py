...
async def lock(ctx: commands.Context):
    ...
    try:
        ...
        await current_channel.set_permissions(ctx.guild.default_role, send_messages=False,
                                              reason=f"Channel locked by {ctx.author.name}")
        await ctx.send(
            f"🔒 **{current_channel.mention} has been locked down.** Members without explicit staff overrides no longer have permission to send messages here.")
    except discord.Forbidden:
...
async def unlock(ctx: commands.Context):
    ...
    try:
        ...
        await current_channel.set_permissions(ctx.guild.default_role, send_messages=None,
                                              reason=f"Channel unlocked by {ctx.author.name}")
        await ctx.send(
            f"🔓 **{current_channel.mention} is now unlocked.** Regular messaging permissions have been restored.")
    except discord.Forbidden:
        await ctx.send("❌ I lack the required permissions to modify permissions on this channel.")
